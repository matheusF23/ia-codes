...
def genetic_algorithm(population_size, crossing_rate=0.8, mutation_rate=0.001, generations=100):
    generation = 0
    ...
    for i, v in enumerate(fitness):
        if (v == max(fitness)):
            plt.figure(num="Aptidão média (azul) e melhor aptidão (laranja) da população")
            plt.plot(generation_list, mean_list, generation_list, max_list)
            plt.title('Aptidão média e melhor aptidão da população')
            plt.ylabel('Aptidão')
            plt.xlabel('Geração')

            # plt.figure(num="Melhor aptidão da população")
            # plt.plot(generation_list, max_list)
            # plt.title(f'Geração {generation}')
            # plt.ylabel('Melhor aptidão da população')
            # plt.xlabel('Geração')

            plt.show()
            return (f'Melhor valor de x: {convert_to_float(population[i])}',
                    f'f(x) = {v}',
